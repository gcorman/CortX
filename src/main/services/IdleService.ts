...
import type { IdleInsight, IdleDraft, IdleAttempt, IdleExplorationEvent, IdleConfig, Entity, Relation } from '../../shared/types'
...
export class IdleService {
  ...
  // AbortController for in-flight LLM calls; reset on each start()
  private stopController: AbortController = new AbortController()
  // Language for LLM prompts and fiche content
  ...
  constructor(
    ...
    if (language) this.language = language === 'en' ? 'en' : 'fr'
  }
  ...
  buildInsightFicheContent(id: string): { subject: string; body: string } {
    ...
    const CATEGORY_LABELS: Record<string, string> = isEn ? {
    ...
    const body = isEn
      ? `**Category**: ${catLabel} — **Confidence**: ${confPct}%
...
---
*Generated by the agent in Idle mode on ${today}*`
      : `**Catégorie** : ${catLabel} — **Confiance** : ${confPct}%
...
Sinon (max 1) : {"promoted": [{"category": "opportunity|development|pattern|contradiction|hidden_connection|gap|cluster", "content": "...", "confidence": 0.0, "entityNames": ["..."], "entityIds": ["..."]}]}`
  ...
  private makeSelectingThought(target: ExplorationTarget): string {
    if (this.language === 'en') {
      if (target.strategy === 'llm') return `↗ Target: ${target.entityNames.slice(0, 2).join(' × ')}`
      if (target.strategy === 'bridge') return `Bridge: ${target.entityNames.join(' ↔ ')}`
      if (target.strategy === 'cluster') return `Cluster: ${target.entityNames.slice(0, 2).join(' + ')}…`
      if (target.strategy === 'peripheral') return `Isolated node: ${target.entityNames[0]}`
      if (target.strategy === 'recent') return `Recent: ${target.entityNames.slice(0, 2).join(' × ')}`
  ...
  private makeExaminingThought(target: ExplorationTarget): string {
    if (this.language === 'en') {
      const names = target.entityNames.slice(0, 2).map((n) => `"${n}"`).join(' and ')
      if (target.strategy === 'llm') return `Targeted analysis of ${names}`
      if (target.strategy === 'bridge') return `Undocumented connection between ${names}?`
      if (target.strategy === 'cluster') return `Pattern around ${names}?`
      if (target.strategy === 'recent') return `Analysis of ${names} (recently modified)`
      return `Examining ${names}`
    }
  ...
  private makeThinkingThought(target: ExplorationTarget): string {
    if (this.language === 'en') {
      const names = target.entityNames.slice(0, 2).join(' and ')
      if (target.strategy === 'llm') return `Potential insight between ${names}…`
      if (target.strategy === 'bridge') return `Opportunity between ${names}?`
      if (target.strategy === 'cluster') return `What does the content of ${names} reveal?`
      if (target.strategy === 'peripheral') return `Underdeveloped aspect around "${target.entityNames[0]}"?`
      if (target.strategy === 'recent') return `New angle in ${names}?`
